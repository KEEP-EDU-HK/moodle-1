...
class block_course_menu extends block_base
{
    ...
    function get_content()
    {
        ...
        // Match the snap theme section parameter (hardcode for SSM)
        $displaysection = optional_param('sec', -1, PARAM_INT);
        //var_dump($this->page->navbar->get_items());
                ...
                foreach ($allSections as $k => $section) {
                    ...
                    if (!isset($this->course->numsections) || $k <= $this->course->numsections) {
                        if (!empty($section)) {
                            ...
                            // Append "sec" for fitting snap theme SSM
                            $newSec['url'] = new moodle_url('/course/view.php?id='.$this->page->course->id.'&sec='.$newSec['id'].'#section-'.$newSec['id']);                            
